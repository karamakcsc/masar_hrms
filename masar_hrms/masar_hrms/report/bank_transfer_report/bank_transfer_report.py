...
def get_data(filters):
	...
	#SQL Query
	data = frappe.db.sql(f"""SELECT te.name AS `Employee Number`, te.full_name_ar AS `Employee Name`, te.department AS `Department`,
							...
							WHERE tss.docstatus = 1 AND tssa.docstatus = 1 AND tss_sub.name = tss.name
										...
										tss.gross_pay, te.iban, te.bank, te.bank_branch, te.bank_ac_no ;""")
